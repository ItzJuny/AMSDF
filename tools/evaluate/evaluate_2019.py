...
def calculate_tDCF_EER(cm_scores_file,
    ...
    asv_keys = asv_data[:, 1]
    asv_scores = np.array(asv_data[:, 2],dtype=float)
    # Load CM scores
    cm_data = np.genfromtxt(cm_scores_file, dtype=str)
    protocal_data = np.genfromtxt(protocol_path, dtype=str)
    cm_data_fn=cm_data[:, 0]
    protocal_data_fn=list(protocal_data[:,1])
    cm2ptc_indes=np.array([protocal_data_fn.index(i) for i in cm_data_fn],dtype=int)
    protocal_data = protocal_data[cm2ptc_indes]
    combine_data = np.column_stack((cm_data,protocal_data))
    cm_sources = combine_data[:, -2]
    cm_keys = ["bonafide" if i =="-" else "spoof" for i in cm_sources]
    cm_keys = np.array(cm_keys,dtype=str)
    cm_scores = np.array(cm_data[:, 1],dtype=float) ### filename,score,label
    ...
    eer_cm_re = compute_eer(-bona_cm, -spoof_cm)[0]#(spoof_cm, bona_cm)[0]
    re_flag=False
    ### reverse
    if eer_cm_re<eer_cm:
        eer_cm=eer_cm_re
        re_flag=True
    attack_types = list(set(cm_sources))
    attack_types.remove('-')
    attack_types.sort()
    spoof_cm_breakdown = {
        attack_type: cm_scores[cm_sources == attack_type] for attack_type in attack_types
    }
    [Pfa_asv, Pmiss_asv,Pmiss_spoof_asv] = obtain_asv_error_rates(tar_asv, non_asv, spoof_asv,asv_threshold)
    if not re_flag:
        # Compute ATTACK EER
        eer_cm_breakdown = {
            attack_type: '%.4f'%(compute_eer(bona_cm, spoof_cm_breakdown[attack_type])[0]) for attack_type in attack_types
        }
        # Compute t-DCF
    ...
    return dict({"eer":'%.4f'%eer_cm, "mtdcf":'%.4f'%min_tDCF}, **eer_cm_breakdown )
...
def compute_det_curve(target_scores, nontarget_scores):
    ...
    nontarget_scores=np.array(nontarget_scores,dtype=float)
